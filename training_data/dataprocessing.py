...
def fix_data(long):
    ...
    # Get all data for the given longitude
        ...
        for i in range(0, 96):
            ...
            if len(interval_data) == 0:
                ...
                processed_data = processed_data._append(
                    {'15 Minutes': mins, 'Lane 1 Flow (Veh/15 Minutes)': int(interval_data.iloc[0])}, ignore_index=True)

    # Save processed data to csv
    processed_data.to_csv(
        f'traffic_flows/{scats_number}_{location_split[1]}_trafficflow.csv', index=False)
...
def merge_datasets():
    ...
    final_data.to_csv(
        f"new_traffic_flows/{scat_site}_trafficflow.csv", index=False)
